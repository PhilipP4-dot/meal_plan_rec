...
OPTIONAL_ROLE_ORDER = ["veg", "sauce", "extra"]

def build_byo_bowl_option(df_bowl, bev_pool,calorie_target, role_limits, used_dishes=None, used_proteins=None, buffer=50, max_attempts=30):
    """
    ...
    vegs     = df_bowl[df_bowl["Role"] == "veg"].copy()
    sauces   = df_bowl[df_bowl["Role"] == "sauce"].copy()
    extras   = df_bowl[df_bowl["Role"] == "extra"].copy()
    ...
    for attempt in range(max_attempts):
        ...
        # fill optional roles
        try_add(vegs, role_limits["veg"])
        try_add(sauces, role_limits["sauce"])
        try_add(extras, role_limits["extra"])
    ...
    # If we never hit the window, return best-so-far OR None
    ...
    chosen, total = best
    if abs(calorie_target - total) <= fallback_buffer and total >= calorie_target * min_fraction:
        return {
            "Hall": hall,
            "Station": "byo_bowl",
            "Calories": int(round(total)),
...
def build_byo_salad_option(df_salad, bev_pool, calorie_target, role_limits, used_dishes=None, used_proteins=None, buffer=50, max_attempts=30):
    """
    df: already filtered to one meal-time + one hall + FinalStation == 'byo_salad'
    calorie_target: target calories for the meal (e.g. 650)
    used_dishes: set of dish names used earlier in the day/week (variety)
    used_proteins: set of protein dish names used earlier (stronger variety)
    buffer: allow exceeding target by up to this many calories
    ...
    # must be same hall+meal already; we'll reuse these for bev filtering
    hall = df_salad["Hall"].iloc[0] if "Hall" in df_salad.columns and len(df_salad) else None
    meal = df_salad["Category"].iloc[0] if "Category" in df_salad.columns and len(df_salad) else None

    proteins = df_salad[df_salad["Role"] == "protein"].copy()
    bases    = df_salad[df_salad["Role"] == "greens_base"].copy()
    vegs     = df_salad[df_salad["Role"] == "veg_topping"].copy()
    sauces   = df_salad[df_salad["Role"] == "dressing"].query("Calories <= 150").copy() # limit heavy dressings that make salad stations chosen often
    extras   = df_salad[df_salad["Role"] == "extra"].copy()
    ...
    for attempt in range(max_attempts):
        ...
        bases["pen"] = bases["Dish"].apply(lambda d: 1 if d in local_used_dishes else 0)
        b_cands = bases.sort_values(["pen", "Calories"]).head(min(8, len(bases))).to_dict("records")
        base = [random.choice(b_cands) for i in range(role_limits["greens_base"])]
        chosen = protein + base
        total = sum(float(p["Calories"]) for p in protein) + sum(float(b["Calories"]) for b in base)
        local_used_dishes.update(b["Dish"] for b in base)
        local_used_proteins.update(p["Dish"] for p in protein)
        ...
        # fill optional roles
        try_add(vegs, role_limits["veg_topping"])
        try_add(sauces, role_limits["dressing"])
        try_add(extras, role_limits["extra"])
...
#print(build_byo_salad_option(df[(df['Hall']=='Huffman') & (df['FinalStation']=='byo_salad') & (df['Category'] == 'Lunch')], bev, 500))
    
def build_byo_breakfast_option(df_breakfast, bev_pool, calorie_target, role_limits, used_dishes=None, used_proteins=None, buffer=50, max_attempts=30):
    """
    df: already filtered to one meal-time + one hall + FinalStation == 'byo_breakfast'
    calorie_target: target calories for the meal (e.g. 650)
    used_dishes: set of dish names used earlier in the day/week (variety)
    used_proteins: set of protein dish names used earlier (stronger variety)
    buffer: allow exceeding target by up to this many calories
    ...
    # must be same hall+meal already; we'll reuse these for bev filtering
    hall = df_breakfast["Hall"].iloc[0] if "Hall" in df_breakfast.columns and len(df_breakfast) else None
    meal = df_breakfast["Category"].iloc[0] if "Category" in df_breakfast.columns and len(df_breakfast) else None

    proteins = df_breakfast[df_breakfast["Role"] == "protein"].copy()
    bases    = df_breakfast[df_breakfast["Role"] == "base"].copy()
    vegs     = df_breakfast[df_breakfast["Role"] == "topping"].copy()
    sauces   = df_breakfast[df_breakfast["Role"] == "sauce"].copy()
    extras   = df_breakfast[df_breakfast["Role"] == "extra"].copy()
    ...
    for attempt in range(max_attempts):
        ...
        # fill optional roles
        try_add(vegs, role_limits["topping"])
        try_add(sauces, role_limits["sauce"])
        try_add(extras, role_limits["extra"])
...
#print(build_byo_breakfast_option(df[(df['Hall']=='Curtis') & (df['FinalStation']=='byo_breakfast') & (df['Category'] == 'Breakfast')], bev, 600))
    ...
    for meal in meal_times:
        ...
        for hall in halls:
            ...
            while safe_max(bowl[bowl["Role"]=="protein"]) * bowl_limits["protein"] + safe_max(bowl[bowl["Role"]=="base"]) * bowl_limits["base"] + safe_max(bowl[bowl["Role"]=="veg"]) * bowl_limits["veg"] \
                + safe_max(bowl[bowl["Role"]=="extra"]) * bowl_limits["extra"] < 0.6 *cal_per_meal and num_of_incr<2:
                if num_of_incr > 0:
            ...
            num_of_incr = 0
            while safe_max(ent[ent["Role"]=="entree"]) * entree_limits["entree"] + safe_max(ent[ent["Role"]=="addon"]) * entree_limits["addon"] < 0.6 *cal_per_meal and num_of_incr < 2:
                if num_of_incr > 0:
            ...
            num_of_incr = 0
            while safe_max(salad[salad["Role"]=="protein"]) * salad_limits["protein"] + safe_max(salad[salad["Role"]=="greens_base"]) * salad_limits["greens_base"] + safe_max(salad[salad["Role"]=="veg_topping"]) * salad_limits["veg_topping"] \
                + safe_max(salad[salad["Role"]=="extra"]) * salad_limits["extra"] < 0.6 *cal_per_meal and num_of_incr < 2:
                if num_of_incr > 0:
            ...
            while safe_max(breakfast[breakfast["Role"]=="protein"]) * breakfast_limits["protein"] + safe_max(breakfast[breakfast["Role"]=="base"]) * breakfast_limits["base"] + safe_max(breakfast[breakfast["Role"]=="topping"]) * breakfast_limits["topping"] \
                + safe_max(breakfast[breakfast["Role"]=="extra"]) * breakfast_limits["extra"] < 0.6 *cal_per_meal and num_of_incr < 2:
                if num_of_incr > 0:
            ...
            for i in range(3):
            # BYO Bowl option
                byo_bowl_option = build_byo_bowl_option(
                    bowl,
                    bev,
                    cal_per_meal,
                ...
                # BYO Salad option
                byo_salad_option = build_byo_salad_option(
                    salad,
                    bev,
                    cal_per_meal,
                ...
                byo_breakfast_option = build_byo_breakfast_option(
                    breakfast,
                    bev,
                    cal_per_meal,
